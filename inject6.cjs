...
const summaryPanelStart = `const SummaryPanel = ({ products, salesHistory, setSalesHistory, expenses, debts, settings, stockHistory, setStockHistory, currentUser }) => {`;
const summaryPanelStartReplacement = `const SummaryPanel = ({ products, salesHistory, setSalesHistory, expenses, debts, settings, stockHistory, setStockHistory, currentUser }) => {
...
// Find the end of SummaryPanel to insert button and portal
// Note: SummaryPanel ends with:
// <Pagination totalItems={filteredStock.length} itemsPerPage={50} currentPage={stockCurrentPage} setCurrentPage={setStockCurrentPage} /></HistoryModal>)}</div>);
